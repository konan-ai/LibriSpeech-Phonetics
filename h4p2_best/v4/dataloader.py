...
from dataset import AudioDataset, ToyDataset
...
def get_toy_dataloader():
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=train_dataset.collate_fn,
                                  shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers)
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, collate_fn=train_dataset.collate_fn,
                                  shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers)
...
if __name__ == "__main__":
    ...
    train, valid = get_training_dataloaders(check=True)
    for i, (mfcc, transcript, mfcc_len, transcript_len) in enumerate(train):
        print(mfcc.shape, transcript.shape, mfcc_len.shape, transcript_len.shape)
        break